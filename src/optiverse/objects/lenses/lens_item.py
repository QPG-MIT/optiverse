...
class LensItem(BaseObj):
    ...
    def __init__(self, params: LensParams, item_uuid: str | None = None):
    ...
    def _maybe_attach_sprite(self):
        ...
        # Check if lens has interfaces - if so, calculate geometry from first interface
    ...
    def _draw_curved_surface(self, p: QtGui.QPainter, p1: QtCore.QPointF, p2: QtCore.QPointF, radius_mm: float):
        """
        Draw a curved lens surface as an arc.
    ...
    def get_interfaces_scene(self):
        ...
        if not interfaces or len(interfaces) == 0:
            # Create default interface from current geometry (backward compatibility)
            p1, p2 = self.endpoints_scene()
            
            default_interface = InterfaceDefinition(
                x1_mm=0.0,  # Scene coordinates used directly
                y1_mm=0.0,
                x2_mm=0.0,
                y2_mm=0.0,
                element_type="lens",
    ...
    @staticmethod
    def from_dict(d: Dict[str, Any]) -> 'LensItem':
        """Static factory method: deserialize from dictionary and return new LensItem."""
