...
def test_save_load_all_components():
    ...
    source = SourceItem(SourceParams(
        x_mm=100.0, y_mm=200.0, angle_deg=45.0,
        wavelength_nm=532.0, polarization_type="vertical"
    ))
    window.scene.addItem(source)
    print("   ✓ Source created (532nm, vertical polarization)")
    
    lens = LensItem(LensParams(
        x_mm=50.0, y_mm=75.0, efl_mm=150.0, name="Test Lens"
    ))
    window.scene.addItem(lens)
    print("   ✓ Lens created (150mm EFL)")
    
    mirror = MirrorItem(MirrorParams(
        x_mm=-50.0, y_mm=-75.0, angle_deg=45.0, name="Test Mirror"
    ))
    window.scene.addItem(mirror)
    print("   ✓ Mirror created")
    
    # Regular beamsplitter
    bs = BeamsplitterItem(BeamsplitterParams(
        x_mm=0.0, y_mm=0.0, split_T=70.0, split_R=30.0, is_polarizing=False
    ))
    window.scene.addItem(bs)
    print("   ✓ Beamsplitter created (70/30 split)")
    
    # PBS (Polarizing Beam Splitter)
    pbs = BeamsplitterItem(BeamsplitterParams(
    ...
    dichroic = DichroicItem(DichroicParams(
        x_mm=200.0, y_mm=300.0, cutoff_wavelength_nm=600.0,
        transition_width_nm=75.0, pass_type="shortpass"
    ))
    window.scene.addItem(dichroic)
    print("   ✓ Dichroic created (600nm cutoff, shortpass)")
    
    # Quarter waveplate
    qwp = WaveplateItem(WaveplateParams(
        x_mm=-100.0, y_mm=-200.0, phase_shift_deg=90.0, fast_axis_deg=45.0
    ))
    window.scene.addItem(qwp)
    print("   ✓ Quarter waveplate created (45° fast axis)")
    
    # Half waveplate
    hwp = WaveplateItem(WaveplateParams(
        x_mm=150.0, y_mm=250.0, phase_shift_deg=180.0, fast_axis_deg=22.5
    ))
    window.scene.addItem(hwp)
    print("   ✓ Half waveplate created (22.5° fast axis)")
    
    slm = SLMItem(SLMParams(
        x_mm=300.0, y_mm=400.0, object_height_mm=100.0, name="Test SLM"
    ))
    ...
    # Find QWP vs HWP
    qwp_loaded = next((wp for wp in waveplates if abs(wp.params.phase_shift_deg - 90.0) < 0.01), None)
    hwp_loaded = next((wp for wp in waveplates if abs(wp.params.phase_shift_deg - 180.0) < 0.01), None)
